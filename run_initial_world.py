...
for repeat in range(EPISODES):
    ...
    while world_state.is_mission_running:
        ...
        if world_state.number_of_observations_since_last_state > 0:
            ...
            steve_loc = (ob.get(u'XPos', 0), ob.get(u'YPos', 0), ob.get(u'ZPos', 0))
            steve.closest_enemy(steve_loc, steve.entities)
            ...
            if steve_agent.check_enemies(ob, mob_type) == 0: #steve clearin arena
            	arena_bonus = 1000
            else:
            	arena_bonus = 0

            mob_distance=(steve.calculate_distance(steve_loc, steve.entities[steve.target])+3)
            reward = (next_state[0]*10 - next_state[4]*20 - time_alive*4 + player_bonus + 
            kill_bonus + arena_bonus - (mob_distance*15)) # get reward
